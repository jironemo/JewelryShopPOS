...
public class AddOrderController {
	...
	public void addOrder(Customer c, Item i) {
		...
		String sql = String.format(pattern, c.getCusID(cus_name.getText()),Item.getItemFromName(item_name.getText()),dp_due.getValue(),description.getText());
		try {
	...
	public void addOrderedItem(Item i) {
		String sql = String.format("INSERT INTO Stock (Name,Weight,Depreciation,stock_status) values ('%s','%s','%s','%s')",i.name,i.weight.getString(),i.depreciation.getString(),i.stock );
		Connection con = Connector.connect();
		try {
			Statement s = con.createStatement();
			s.execute(sql);
